...
export class H1Connection {
	...
	#onHeaders(m) {
		...
		const contentLengths = m.contentLengths ?? EMPTY_VALUES;
		const declaredTrailers = (m.declaredTrailerValues ?? EMPTY_VALUES).flatMap(
			(value) =>
				value
					.toLowerCase()
					.split(",")
		...
		const connectionTokens = (m.connectionValues ?? EMPTY_VALUES).flatMap(
			(value) =>
				value
					.toLowerCase()
					.split(",")
		...
		const transferTokens = (m.transferEncodings ?? EMPTY_VALUES).flatMap(
			(value) =>
				value
					.toLowerCase()
					.split(",")
		...
		const expect = (m.expectValues ?? EMPTY_VALUES).join(", ").toLowerCase();
