...
export class H1Connection {
	...
	#onHeaders(m) {
		...
		const values = (field) =>
			m.headers
				.filter(([name]) => name.toLowerCase() === field)
				.map(([, value]) => value);
		const hosts = values("host");
		const contentLengths = values("content-length");
		const transferEncodings = values("transfer-encoding");
		const declaredTrailers = values("trailer").flatMap((value) =>
			value
				.toLowerCase()
				.split(",")
				.map((token) => token.trim()),
		);
		const connectionTokens = values("connection").flatMap((value) =>
			value
				.toLowerCase()
				.split(",")
				.map((token) => token.trim()),
		);
		const transferTokens = transferEncodings.flatMap((value) =>
			value
				.toLowerCase()
				.split(",")
				.map((token) => token.trim()),
		...
		if (
			hosts.length !== 1 ||
			hosts[0].trim() === "" ||
			contentLengths.some((value) => !/^\d+$/.test(value.trim())) ||
			(contentLengths.length > 1 &&
				!contentLengths.every(
					(value) => value.trim() === contentLengths[0].trim(),
				)) ||
			(contentLengths.length > 0 && transferTokens.length > 0) ||
			(transferTokens.length > 0 &&
				(transferTokens.length !== 1 || transferTokens[0] !== "chunked")) ||
			declaredTrailers.some((name) => FORBIDDEN_TRAILER_FIELDS.has(name)) ||
			connectionTokens.some((token) =>
				["content-length", "host", "transfer-encoding"].includes(token),
			)
		) {
			this.#onParseError();
			return;
		}
		const headerBytes = m.headers.reduce(
			(total, [name, value]) =>
		...
		if (
			...
			headerBytes > this.#limits.maxHeaderSize
		) {
		...
		if (m.method === "CONNECT") {
			if (m.url.toLowerCase() !== hosts[0].trim().toLowerCase()) {
				this.#onParseError();
				return;
			}
			this.#socket.pause();
		...
		if (m.url.startsWith("/")) {
			...
			if (
				...
				absolute.host.toLowerCase() !== hosts[0].trim().toLowerCase()
			) {
				this.#onParseError();
				return;
			}
		...
		// Request construction is lazy, so retain its eager URL validation without
		// allocating the Request itself on handlers that never access it.
		if (!URL.canParse(url)) {
			this.#onParseError();
			return;
		}
		...
		const expect = values("expect").join(", ").toLowerCase();
	...
	#onData(chunk) {
		...
		if (
			...
			entry.abort.abort(new Error("request body too large"));
			this.#reject(413, "Content Too Large");
			return;
		}
	...
	#remoteAddress() {
		return {
			...
			port: this.#socket.remotePort ?? null,
			family: this.#socket.remoteFamily ?? null,
		};
	}
	...
	async #pump() {
		...
		try {
			while (this.#queue.length > 0 && !this.#closed) {
				...
				this.#timerState = -1;
				this.#setTimer(ACTIVE);
				const response = await this.#dispatch(entry.getRequest, {
					remoteAddress: this.#remoteAddress(),
					httpVersion: "1.1",
					alpnProtocol: this.#socket.alpnProtocol || null,
	...
	#teardown(err) {
		...
		for (const entry of this.#queue) {
			if (!entry.responded) entry.abort.abort();
