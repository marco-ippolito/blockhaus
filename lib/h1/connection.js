...
export class H1Connection {
	...
	#onHeaders(m) {
		...
		for (const [name, value] of m.headers) headers.append(name, value);
		if (m.method === "CONNECT") {
			...
			this.#socket.pause();
			this.#pendingConnect = { authority: m.url, headers };
			return;
		}
		...
		const expect = (headers.get("expect") ?? "").toLowerCase();
	...
	async #pump() {
		...
		try {
			while (this.#queue.length > 0 && !this.#closed) {
				...
				this.#timerState = -1;
				this.#setTimer(ACTIVE);
				const response = await this.#dispatch(entry.request, {
					remoteAddress: this.#remoteAddress(),
					httpVersion: "1.1",
					alpnProtocol: this.#socket.alpnProtocol || null,
