	...
	on_header_value(parser, at, len) {
		...
		switch (normalizedName) {
			case "host":
				...
				break;
			case "content-length":
				if (m.contentLengths === null) m.contentLengths = [];
				m.contentLengths.push(value);
				m.contentLength = Number.parseInt(value, 10) || 0;
				break;
			case "transfer-encoding":
				if (m.transferEncodings === null) m.transferEncodings = [];
				m.transferEncodings.push(value);
				m.chunked ||= value.toLowerCase().includes("chunked");
				break;
			case "trailer":
				if (m.declaredTrailerValues === null) m.declaredTrailerValues = [];
				m.declaredTrailerValues.push(value);
				break;
			case "connection":
				if (m.connectionValues === null) m.connectionValues = [];
				m.connectionValues.push(value);
				...
				break;
			case "expect":
				if (m.expectValues === null) m.expectValues = [];
				m.expectValues.push(value);
				break;
		}
		p.pendingHeaderName = "";
	},
