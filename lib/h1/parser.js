...
const milo = setup({
	...
	on_header_value(parser, at, len) {
		...
		p.message.headers.push([p.pendingHeaderName, str(at, len)]);
		p.pendingHeaderName = "";
	},
	...
	on_headers(parser) {
		...
		for (const [name, value] of m.headers) {
			...
			if (n === "content-length") {
				contentLength = Number.parseInt(value, 10) || 0;
			} else if (n === "transfer-encoding") {
				chunked ||= value.toLowerCase().includes("chunked");
			} else if (n === "connection") {
				for (const token of value.toLowerCase().split(",")) {
					const t = token.trim();
					if (t === "close") m.connectionClose = true;
					else if (t === "upgrade") m.upgrade = true;
				}
			}
		}
		m.hasBody = chunked || contentLength > 0;
