...
async function handleStream(
	...
	for (const name in headers) {
		...
		if (Array.isArray(value)) {
			for (const v of value) {
				headerBytes += name.length + Buffer.byteLength(v) + 4;
			}
		} else if (value !== undefined) {
			headerBytes += name.length + Buffer.byteLength(String(value)) + 4;
		}
	}
	...
	let connectionMeta = session && sessionMetadata.get(session);
	if (connectionMeta === undefined) {
		connectionMeta = {
			remoteAddress: {
		...
		if (session) sessionMetadata.set(session, connectionMeta);
	}
	...
	let response = dispatch(getRequest, {
		remoteAddress: connectionMeta.remoteAddress,
		scheme,
		authority,
		path,
		method,
		...
		httpVersion: "2",
		alpnProtocol: connectionMeta.alpnProtocol,
		signal: getSignal,
	...
	try {
		while (true) {
			...
			// A BodyInit-backed response closes one microtask after yielding its
			// sole chunk. Observing reader.closed avoids a redundant read request
			// while preserving arbitrary multi-chunk streaming behavior.
			await 0;
