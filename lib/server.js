...
import { diagnosticChannels, publishDiagnostic } from "./diagnostics.js";
...
export class Server {
	...
	#dispatch = async (request, meta) => {
		...
		const ctx = new Context(request, {
			...meta,
			waitUntil: (promise) =>
				this.#trackWaitUntil(promise, { request, requestId }),
		});
		let response;
		try {
			response = await this.#handler.fetch(ctx);
		} catch (error) {
			this.#report(error, { phase: "handler", request, requestId });
			if (ctx.denied) {
